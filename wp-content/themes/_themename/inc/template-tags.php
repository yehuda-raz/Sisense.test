...
if ( ! function_exists( '_themename_posted_on' ) ) :
	...
	function _themename_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', '_themename' ),
	...
	function _themename_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', '_themename' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', '_themename' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
